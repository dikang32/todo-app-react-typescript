...
function Home() {
  ...
  //  Add Task function
  const [task, setTask] = useState<IState['task']>({
    id: "",
    title: "",
    priority: "medium",
    status: "Todo",
  ...
  // Delete Task
  const [selectTaskIndex, setSelectTaskIndex] = useState<number | null>(null)
  const handleDelete = (x: number) => {
    setOpenDeleteModal(true);
    setSelectTaskIndex(x)
  }
  ...
  const handleEdit = (task: { id: string; title: string; priority: string; status: string; }) => {
  ...
  const handleChangeStatus = (index: number) => {
    ...
    const currentStatus = updatedTaskList[index].status;
    const nextStatus = statusOrder[(statusOrder.indexOf(currentStatus) + 1) % statusOrder.length]
    updatedTaskList[index].status = nextStatus;
  ...
  const getStatusCircle = (status: string) => {
    switch (status) {
      ...
      case "In Progress":
        return (
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
            <path d="M12 23.5C18.0751 23.5 23 18.5751 23 12.5C23 6.42487 18.0751 1.5 12 1.5C5.92487 1.5 1 6.42487 1 12.5C1 18.5751 5.92487 23.5 12 23.5Z" stroke="#713FFF" stroke-width="2" />
            <path d="M12 1.5C5.92487 1.5 1 6.42487 1 12.5C1 18.5751 5.92487 23.5 12 23.5" stroke="#E5E6E9" stroke-width="2" />
          </svg>
        );
      case "Done":
        return (
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none">
            <path d="M12 2.5C17.5228 2.5 22 6.97715 22 12.5C22 18.0228 17.5228 22.5 12 22.5C6.47715 22.5 2 18.0228 2 12.5C2 6.97715 6.47715 2.5 12 2.5Z" stroke="#713FFF" stroke-width="2" />
          </svg>
        );
      default:
        return (
  ...
  return (
    <>
      ...
      ></ModalEditTask>}
      {openDeleteModal && <ModalDeleteTask
        openDeleteModal={openDeleteModal}
        setOpenDeleteModal={setOpenDeleteModal}
        selectTaskIndex={selectTaskIndex}
        setSelectTaskIndex={setSelectTaskIndex}
        taskList={taskList}
        setTaskList={setTaskList}
      ></ModalDeleteTask>}
      <div className={styles["contaner-header"]}>
          ...
          {taskList.map((task, index) => (
            <div className={styles.list__item} key={index}>
              ...
              <div className={styles.list__priority}>
                <h5>Priority</h5>
                <p style={{ color: getColor(task.priority) }}>{task.priority}</p>
              </div>
              <button className={styles.list__progress} onClick={() => handleChangeStatus(index)}>
                {task.status}
              </button>
              <div className={styles.list__circle}>
                {getStatusCircle(task.status)}
              </div>
              <div className={styles["list__group-button"]}>
                <button className={styles["list__edit-button"]} onClick={() => handleEdit(task)}>
                ...
                <button className={styles["list__delete-button"]} onClick={() => handleDelete(index)}>
